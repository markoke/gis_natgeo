---
title: "LAND USE AND LAND COVER DETECTION"
output: html_notebook
---
### Loading required libraries
This is where the required libraries/ packages are loaded.
...
```
### Setting varibles and starting to process
The required files and variables will be set and the process will start.
```{r Set variables and start processing}
# Set working directory
setwd("C:/Users/Brian Andrew/Desktop/Project")
# Name and path for the Shapefile (don't need the .shp extension)
shapefile <- "C:/Users/Brian Andrew/Desktop/Project/test4/training.shp"
...
inImageName <-"C:/Users/Brian Andrew/Desktop/Project/test4/mfnp_2011_2014_stacked_upper.tif"
...
yBand <- 3
```
### Setting the start processing time and creating land cover attributes
```{r Start processing}
startTime <- Sys.time()
...
### Creating input data from shapefile and creating training data
```{r Start processing_1}
...
### Testing if feature space plot is needed and calculating the output layers selected in the variables section
```{r Start processing_2}
...
if (xBand != 0 & yBand != 0) {
  ...
  while (continue == "c") {
    ...
    #This sets the prompt that has to be entered at the console and press enter to continue
...
### Processing the raster output

```{r Start processing_3}
# Remove NA values 
...
if (nrow(trainvals) == 0) {
  ...
  stop("It is possible the projection of the Shapefile with training data and input image are different. Check projections and run again", call.=FALSE)
}
...
```
### Plotting the image output
```{r Plot image, fig.align='center'}
...
varImpPlot(randfor)
```
### Fanially calculating the processing time
```{r Calculating processing time}
